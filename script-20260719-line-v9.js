...
function switchView(name){["shifts","staff","settings","data","appSettings"].forEach(v=>$(v+"View").classList.toggle("hidden",v!==name));document.querySelectorAll(".tab-button").forEach(b=>b.classList.toggle("active",b.dataset.view===name))}
function openModal(name){$(name+"Modal").classList.remove("hidden")}
function closeModal(name){$(name+"Modal").classList.add("hidden")}
function updateFirstSetupPanel(){const panel=$("firstSetupPanel");if(!panel)return;const titleEl=$("staffCountTitle");if(titleEl)titleEl.textContent=`現在スタッフ登録 ${state.staff.length}人`;panel.classList.toggle("hidden",state.staff.length>0)}
function renderAll(){renderTypeSelects();renderShiftList();renderStaffList();renderCategoryList();renderAutoBackupList();updateFirstSetupPanel();if(selectedShiftId)renderDetail()}
...
function saveShift(e){
 e.preventDefault();
 const name=els.shiftName.value.trim(),type=els.shiftType.value,start=els.startDate.value,end=els.endDate.value;
 ...
 let created=null;
 if(existing){Object.assign(existing,{name,type,startDate:start,endDate:end});normalize(existing)}
 else{created={id:id(),name,type,startDate:start,endDate:end,assignments:{},staffOverrides:{include:[],exclude:[]},showHeadcount:false,lineSharedAt:null};state.shifts.unshift(created)}
 save();closeModal("shift");renderAll();
 if(created){
   const previous=state.shifts.find(s=>s.id!==created.id&&s.type===created.type);
   if(previous&&confirm("前回シフトをコピーしますか？\n［キャンセル］を選ぶと白紙から作成します。")){copyShiftInto(previous,created);save();renderAll();toast("前回シフトをコピーしました")}
...
function copyShiftInto(source,target){
 ...
 Object.entries(source.assignments||{}).forEach(([staffId,values])=>{
   ...
   targetDates.forEach((date,i)=>{const sourceDate=sourceDates[i];const value=sourceDate?values?.[sourceDate]:"";if(value)target.assignments[staffId][date]=value});
...
function saveStaff(e){e.preventDefault();const name=els.staffName.value.trim();if(!name)return;const workTypes=[...els.staffWorkTypes.querySelectorAll('input:checked')].map(x=>x.value);if(!workTypes.length)return alert("勤務パターンを1つ以上選んでください。");const p=state.staff.find(x=>x.id===els.staffId.value);if(p){p.name=name;p.workTypes=workTypes;p.isManager=els.isManager.checked}else state.staff.push({id:id(),name,workTypes,isManager:els.isManager.checked,order:state.staff.length});sortStaff();save();closeModal("staff");renderAll();toast("スタッフを保存しました")}
...
function addCategory(){const name=els.newCategoryName.value.trim();if(!name)return alert("勤務パターン名を入力してください。");if(state.categories.some(c=>c.name===name))return alert("同じ勤務パターン名が登録されています。");const category={id:slugifyCategory(name),name};state.categories.push(category);state.customOptions[category.id]=[];els.newCategoryName.value="";save();renderAll();toast(`「${name}」を追加しました`)}
function renderCategoryList(){
 ...
 state.categories.forEach((c,i)=>{
  ...
  row.innerHTML=`<div class="category-name-edit"><span class="category-number">${i+1}</span><input class="category-name-input" type="text" maxlength="20" value="${esc(c.name)}" aria-label="勤務パターン名"></div><div class="staff-actions"><button type="button" class="primary-button save-name">変更を保存</button><button type="button" class="danger-button del">削除</button></div>`;
...
function renameCategoryFromInput(c,value){
 ...
 if(!name)return alert("勤務パターン名を入力してください。");
 if(name===c.name)return toast("変更はありません");
 if(state.categories.some(x=>x.id!==c.id&&x.name===name))return alert("同じ勤務パターン名が登録されています。");
 c.name=name;save();renderAll();toast("勤務パターン名を変更しました");
}
function renameCategory(c){const name=prompt("新しい勤務パターン名を入力してください。",c.name)?.trim();if(!name||name===c.name)return;renameCategoryFromInput(c,name)}
function deleteCategory(c){if(state.categories.length<=1)return alert("勤務パターンは最低1つ必要です。");const usedShift=state.shifts.some(s=>s.type===c.id),usedStaff=state.staff.some(p=>(p.workTypes||[]).includes(c.id));if(usedShift||usedStaff)return alert("この勤務パターンはシフト表またはスタッフで使用中です。先に別の勤務パターンへ変更してください。");if(!confirm(`「${c.name}」を削除しますか？`))return;state.categories=state.categories.filter(x=>x.id!==c.id);delete state.customOptions[c.id];save();renderAll();toast("勤務パターンを削除しました")}
