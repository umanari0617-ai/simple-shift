...
async function shareToLine(){
 ...
 try{
  if(navigator.share){
   await navigator.share({title:"シンプルシフト表｜飲食店版",text,url});
   toast("共有画面を開きました");
   return;
  }
  const lineUrl=`https://line.me/R/msg/text/?${encodeURIComponent(text)}`;
  const opened=window.open(lineUrl,"_blank","noopener");
  ...
  try{
   if(navigator.clipboard&&window.isSecureContext){
    ...
    alert("LINEの共有画面を開けませんでした。共有内容をコピーしました。LINEを開いて貼り付けてください。");
   }else{
    throw new Error("clipboard-unavailable");
   }
  }catch{
   alert("共有できませんでした。SafariまたはChromeで開き直して、もう一度お試しください。");
  }
 }
}
